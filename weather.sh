...
#temp_sign="º""$2"
    ...
    forcast=`echo "$html" | grep 'yweather:forecast' -m $3 | awk -F\"  -v SIGN="$temp_sign"  '{print $4" "$2" "$6" "SIGN" ~ "$8" "SIGN" "$10 }'`
    
#forcast=`echo "$html" | grep 'yweather:forecast' -m $3 | awk -v SIGNaaa="zz" '{print SIGNaaa}' `
...
#    create  default file
#    echo "$home_dir/.weather.conf doesn't exist!"
#    echo "Create default configure file!"
#    echo "location=hsinchu" >> $home_dir/.weather.conf
#    echo "unit=c" >> $home_dir/.weather.conf
else
